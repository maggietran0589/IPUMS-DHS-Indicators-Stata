...
keep clusterno hhnum v003 kiddobcmc_all linenokid_all
* drop if the child in the birth history was not in the household or not alive 
drop if linenokid_all==0 | linenokid_all==.
* rename key variables for matching 
rename linenokid_all  hhlineno
rename clusterno clusternoall
...
* gen dup = (clusternoall == clusternoall[_n-1] & hhnumall == hhnumall[_n-1] & hhlineno == hhlineno[_n-1])
* drop if dup==1
* drop dup
...
merge 1:1 clusternoall hhnumall hhlineno using tempBR
...
gen school_age = int((cmcSch - kiddobcmc_all) / 12) if kiddobcmc_all != .
...
gen xtemp = hhintcmc - (hhage * 12) if kiddobcmc_all == .
gen cmctemp = xtemp - int(uniform()*12) if kiddobcmc_all == .
replace school_age = int((cmcSch - cmctemp) / 12) if kiddobcmc_all == .
