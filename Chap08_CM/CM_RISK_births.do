...
kidtwin           “Child is a twin or single birth”
/*----------------------------------------------------------------------------
...
* mother's age
gen age_of_mother=int((kiddobcmc-dobcmc)/12)

* Adjustment for multiple births to give the same order as that of the first in multiples;
* kidtwin is sequence in the multiple birth if part of a multiple birth; kidtwin=0 if not a multiple birth;
* only shift the second (or later) birth within a multiple birth.
gen bord_adj=kidbord
replace bord_adj=kidbord-kidtwin+1 if kidtwin>1
...
replace soon=1 if birthintpremo<24
...
*mean cm* [iweight=perweight/1000000]
...
proportion kidalive if cm_riskb_none==1 [iweight=perweight/1000000]
matrix T=r(table)
scalar denom=T[1,1]
...
quietly foreach lcat in $risk_categories {
	proportion kidalive if `lcat'==1 [iweight=perweight/1000000]
	matrix T=r(table)
	scalar num=T[1,1]
	scalar `lcat'_RR=round(num/denom,.01)
	
	qui proportion kidalive if `lcat'==1
	scalar Nobs=e(N)
